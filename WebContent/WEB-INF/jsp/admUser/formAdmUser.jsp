...
<form action="<c:url value="/adm/user" />" method="post" id="formAdmUser">
	...
	<table cellpadding="0" cellspacing="0">
		...
		<tr>
			<td><label for="login" class="label">Login: </label>
			</td>
		...
		<tr>
			<td><label for="password" class="label">Password: </label>
			</td>
		...
		<tr>
			<td><label for="userBlock" class="label">Bloqueado: </label>
			</td>
		...
		<tr>
			...
			<td><c:forEach var="admPermission" items="${admPermissions}"
		...
		<tr>
			<td colspan="2"><c:if test="${admUser.id == null}">
					<button name="_method" class="submit botoes" value="post"></button>
				</c:if> <c:if test="${admUser.id != null}">
...
<c:if test="${admUser != null}">
	...
	<form method="post" action="<c:url value="/adm/user/${admUser.id}" />"
		id="formDeleteAdmUser">
		<input name="btnDeleteAdmUser" id="btnDeleteAdmUser"
			class="botoes delete" value="" type="button" /> <input
			name="_method" id="_method" value="delete" type="hidden" />
	</form>
